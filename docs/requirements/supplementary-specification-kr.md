...
- `start()` 전과 `stop()` 후에는 simulator tick과 console tick counter가 진행되지 않도록 running-state guard를 유지한다.
...
- 기존 start, stop, dust, normal tick, start 전 idle tick 동작은 유지되어야 한다.
