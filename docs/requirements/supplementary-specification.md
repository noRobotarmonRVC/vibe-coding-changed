...
- Kept simulator ticks inactive before `start()` and after `stop()` so the console clock and simulated sensor effects do not advance while idle.
...
| TEST-05 | Existing start, stop, dust, normal tick, and idle-before-start behaviors must remain intact. |
