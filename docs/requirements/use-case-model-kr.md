...
| 1.1 | 2026-05-29 | Right Sensor 액터 삭제; UC-02/03/04에서 우측 센서 읽기 제거 — 우측은 우회전 후 전방 센서로 확인 (AD-11) |
...
| Front Sensor | External System | 정면의 장애물을 감지; interrupt 방식. 우회전 후 우측 확인에도 재활용. |
...
Front Sensor ──────────────────────> UC-03: 전방 장애물 회피
                                     UC-04: 포위 상태 탈출 (우측 확인)
Left Sensor ───────────────────────> UC-04: 포위 상태 탈출
...
2. 시스템이 Left Sensor, Dust Sensor 상태를 읽는다.
...
| 전방 차단 AND 좌측 차단 AND 우측(우회전 확인) 차단 | → UC-04: 포위 상태 탈출 |
...
| **사전 조건** | 청소 세션이 활성 상태. Front Sensor가 True를 발생시킴. 좌측 또는 (우회전으로 확인한) 우측이 개방되어 있음. |
...
3. 시스템이 Left Sensor를 읽는다. 좌측이 개방이면 좌회전한다.
4. 좌측이 차단이면 시스템이 우회전한 뒤 Front Sensor로 우측을 확인한다.
5. 우측이 개방이면 (이미 우측을 향한 상태로) 전진을 재개한다.
6. Cleaner는 On 상태를 유지한다.

**대안 흐름 — 양쪽 모두 차단:**

- 4단계: 좌측 차단 AND 우측(우회전 확인) 차단 → UC-04로 이전.
...
| **간략 설명** | 전방·좌측이 차단되고 (전방 센서 회전으로 확인한) 우측도 차단되면, RVC가 한 칸 후진하고 회전한 뒤 재개한다. |
| **사전 조건** | 청소 세션이 활성 상태. Front = True AND Left = True AND 우측(우회전 확인) = True. |
| **사후 조건** | RVC가 새로운 방향으로 전진 중이며, Cleaner는 On 상태를 유지한다. |

**주요 성공 시나리오:**

1. 시스템이 전방 차단, 좌측 차단, (우회전으로 확인한) 우측 차단을 감지한다.
2. 시스템이 Motor에 Backward 명령을 내린다 (한 칸).
3. 시스템이 회전 방향을 선택한다.
